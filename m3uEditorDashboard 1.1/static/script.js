document.addEventListener('DOMContentLoaded', () => {
    ...
    const xtreamPanelUrlInput = document.getElementById('xtreamPanelUrl');
    ...
    const xtreamOutputTypeSelect = document.getElementById('xtreamOutputType');
    const loadFromXtreamButton = document.getElementById('loadFromXtreamButton');
    ...
    if (loadFromXtreamButton) loadFromXtreamButton.addEventListener('click', handleLoadFromXtream);
    ...
    async function handleLoadFromUrl() {
        ...
        commonInitialClearAndSetup();
        toggleLoading(true, "main");
        loadingMessage.textContent = `Fetching playlist from URL via proxy...`;
    ...
    async function handleLoadFromXtream() {
        const panelUrl = xtreamPanelUrlInput.value.trim();
        const username = xtreamUsernameInput.value.trim();
        const password = xtreamPasswordInput.value;
        const outputType = xtreamOutputTypeSelect.value;

        if (!panelUrl || !username || !password) {
            alert("Please enter Xtream Panel URL, Username, and Password.");
            return;
        }

        commonInitialClearAndSetup();
        toggleLoading(true, "main");
        loadingMessage.textContent = `Fetching playlist from Xtream panel...`;

        try {
            const backendResponse = await fetch('/fetch_xtream_playlist', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!backendResponse.ok || !result.success) {
                throw new Error(result.error || `Failed to fetch from Xtream. Server responded with status: ${backendResponse.status}`);
            }
            
            const xtreamM3uContent = result.m3uContent;
            ...
            console.error("Error loading from Xtream panel:", error);
            let displayError = error.message;
             if (error.message.includes("Failed to fetch")) {
                displayError = "Network error connecting to the server. Please check your connection.";
            }
            setChannelPanePlaceholder(`Failed to load from Xtream: ${displayError}`, true);
            toggleLoading(false, "main");
            disableActionButtons();
        }
    }
    ...
    function renderChannelsPane(categoryName) {
        ...
        if (filteredChannels.length === 0) {
            ...
            filteredChannels.forEach((channel) => {
                ...
                channelNameSpan.addEventListener('click', () => toggleButton.click()); // Make name clickable to toggle
    ...
    function hideChannelOverlay() { // Dummy or basic implementation
        const overlay = document.getElementById('channelOverlay');
        if (overlay) overlay.style.display = 'none'; // Or toggle a class like .overlay-visible
    }
